...
The Blueprint currently uses Render's `free` plans so the project can be deployed without assuming a paid budget. Render currently documents that Free Postgres databases expire 30 days after creation and do not provide backups; Free web services also have resource and availability limitations. Treat this configuration as **demo/staging**, not a durable production deployment. Upgrade the web service and Postgres plan before using MORPH for real operational data. See the current Render free-tier documentation for the exact limits.
...
- PostgreSQL is used whenever `DATABASE_URL` is set, preventing Render web-service restarts from losing application state.
...
- Authenticated users can access their own documents/outputs; ADMIN and OFFICER roles can access shared operational data.
