...
export async function parseUploadedContent({ name = "Pasted source", type = "text/plain", content = "", encoding = "text" }) {
  ...
  return {
    title: inferTitle(clean, name), text: clean, fileType: ext || "txt", supported,
...
export function analyzeDocument(doc, chunks) {
  ...
  const claims = sentences.slice(0, 30).map((claim, idx) => ({ id: id("fact"), documentId: doc.id, claim, citationId: `C${idx + 1}`, page: chunks[Math.min(idx, chunks.length - 1)]?.page || 1, section: chunks[Math.min(idx, chunks.length - 1)]?.section || "Source", confidence: claim.match(/\d|must|shall|eligible|deadline|budget|launched/i) ? 0.94 : 0.84, createdAt: now() }));
...
function extractEntities(text, documentId) {
  const matches = [...text.matchAll(/\b(?:[A-Z][a-z]+|[A-Z]{2,})(?:\s+(?:[A-Z][a-z]+|[A-Z]{2,})){0,5}\b/g)].map((m) => m[0]).filter((x) => x.length > 2 && !/^(The|This|If|For|Section|Issued)$/.test(x));
...
export function retrieve(chunks, question, limit = 4) {
  ...
  return chunks.map((chunk) => { const hay = chunk.text.toLowerCase(); const score = [...terms].reduce((sum, term) => sum + (hay.includes(term) ? 1 : 0), 0); return { ...chunk, score }; }).sort((a, b) => b.score - a.score).slice(0, limit);
}
