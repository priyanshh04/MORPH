...
function render(type,c,length){const list=(a,n=6,p="• ")=>a.length?a.slice(0,n).map(f=>`${p}${f.text} ${f.marker}`).join("\n"):"• Not explicitly stated in the analyzed source.";const dates=c.dates.length?c.dates.slice(0,8).map(x=>`• ${x}`).join("\n"):"• No explicit date found.";const amounts=c.amounts.length?c.amounts.slice(0,8).map(x=>`• ${x}`).join("\n"):"• No explicit amount found.";const p=c.facts.slice(0,length==="Detailed"?10:length==="Short"?4:7);switch(type){case"Citizen Simplifier":return`WHAT THIS MEANS\n${c.title}\n\nKEY POINTS\n${list(p)}\n\nAUDIENCE\n${list(c.eligibility,4)}\n\nBENEFITS / RESULTS\n${list(c.benefits,4)}\n\nDATES\n${dates}\n\nWHAT TO DO\n${list(c.actions,4)}`;case"Officer Brief":return`OFFICER BRIEF\n${c.title}\n\nSITUATION / RELEASE\n${list(c.launch.length?c.launch:p,3)}\n\nKEY FACTS\n${list(p,7)}\n\nREQUIREMENTS\n${list(c.actions,6)}\n\nDATES / DEADLINES\n${dates}`;case"Executive Summary":return`EXECUTIVE SUMMARY\n${c.title}\n\nCONTEXT\n${list(p,4)}\n\nKEY FACTS\n${list(p,7)}\n\nNUMERIC / FINANCIAL FIGURES\n${amounts}\n\nKEY DATES\n${dates}`;case"FAQ Generator":return faq(c);case"WhatsApp Generator":return`SOURCE UPDATE — ${c.title}\n\n${list(p,4)}\n\nAUDIENCE\n${list(c.eligibility,2)}\n\nDATES\n${dates}`;case"Social Media Generator":return`SOCIAL MEDIA DRAFT\n${c.title}\n\n${list(p,4)}\n\nWHY IT MATTERS\n${list(c.benefits.length?c.benefits:p,3)}\n\nDATE / DEADLINE\n${dates}`;case"Presentation Generator":returnslides(c);case"Voice Script":return`VOICE SCRIPT\n\nThis briefing covers ${c.title}.\n\n${list(p,4,"")}\n\nAUDIENCE / RESULT\n${list(c.eligibility.length?c.eligibility:c.benefits,3,"")}\n\nIMPORTANT DATE\n${c.dates[0]||"No explicit date stated."}`;case"Press Release":return`PRESS RELEASE DRAFT\n${c.title}\n\nHEADLINE\n${shorten((c.launch[0]||c.facts[0]||{text:c.title}).text,120)}\n\nDETAILS\n${list(p,6)}\n\nDATES\n${dates}`;case"SMS / Alert":return`SMS / ALERT\n${shorten((c.deadlines[0]||c.actions[0]||c.launch[0]||c.facts[0]||{text:FALLBACK}).text,160)}`;case"Infographic Content":return`INFOGRAPHIC CONTENT\n${c.title}\n\nKEY FACTS\n${list(p,5)}\n\nNUMBERS / MONEY\n${amounts}\n\nDATES\n${dates}`;case"Detailed Report":return`DETAILED SOURCE REPORT\n${c.title}\n\nOVERVIEW\n${list(p,8)}\n\nAUDIENCE\n${list(c.eligibility,6)}\n\nRESULTS / FINANCIALS\n${list(c.benefits,6)}\n${amounts}\n\nDATES / DEADLINES\n${dates}\n\nACTIONS\n${list(c.actions,6)}\n\nSOURCE LIMITATION\nNo absent facts are inferred.`;case"Key Facts / Statistics":return`KEY FACTS & STATISTICS\n${c.title}\n\nNUMBERS / AMOUNTS\n${amounts}\n\nDATES / YEARS\n${dates}\n\nKEY FACTS\n${list(p,8)}`;case"Action Checklist":return`ACTION CHECKLIST\n${c.title}\n\n${list(c.actions.length?c.actions:p,8,"☐ ")}\n\nDATES\n${dates}`;default:return`SOURCE-GROUNDED OUTPUT\n${c.title}\n\n${list(p,7)}`;}}
function faq(c){const a=[];const add=(q,f)=>f&&a.length<6&&a.push(`Q${a.length+1}. ${q}\nA. ${f.text} ${f.marker}`);add("When was it launched or released?",c.launch.find(f=>hasDate(f.text))||c.launch[0]);add("Who is the audience or affected group?",c.eligibility[0]);add("What result, benefit or feature is described?",c.benefits[0]);add("What price, amount, rating or metric is stated?",c.facts.find(f=>matches(f.text,MONEY_RE).length||/rating|score|price|sales|revenue|profit/i.test(f.text)));for(const f of c.facts)add(`What does the source say about ${subject(f.text)}?`,f);return`FREQUENTLY ASKED QUESTIONS\n${c.title}\n\n${a.length?a.join("\n\n"):FALLBACK}`;}
function slides(c){const groups=[["Key facts",c.facts.slice(0,3)],["Audience",c.eligibility.slice(0,3)],["Results / figures",c.benefits.slice(0,3)],["Dates",c.deadlines.length?c.deadlines.slice(0,3):c.launch.slice(0,3)],["Actions",c.actions.slice(0,3)]];return`PRESENTATION OUTLINE\n\n${groups.filter(x=>x[1].length).map((x,i)=>`Slide ${i+1} — ${x[0]}\n${x[1].map(f=>`• ${f.text} ${f.marker}`).join("\n")}`).join("\n\n")}`;}
