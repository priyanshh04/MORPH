...
const server = http.createServer(async (req, res) => {
  try {
    ...
    console.error(error);
    return json(res, error.message === "Payload too large" ? 413 : 500, { error: error.message === "Payload too large" ? error.message : "Internal server error" });
  }
});
...
async function register(req, res) {
  ...
  if (!email || password.length < 8) return json(res, 400, { error: "Email and a password of at least 8 characters are required" });
  const created = await mutateDb((db) => {
    if (db.users.some((u) => u.email.toLowerCase() === email)) return null;
    const user = { id: id("user"), name: String(body.name || "MORPH User").trim().slice(0, 100), email, role: "USER", passwordHash: hashPassword(password), createdAt: now() };
    db.users.push(user);
    return user;
  });
  if (!created) return json(res, 409, { error: "User already exists" });
  return json(res, 201, { token: signToken({ sub: created.id, role: created.role, email: created.email }), user: safeUser(created) });
}

async function documents(res) {
  const db = await readDb();
  return json(res, 200, { documents: db.documents.map((d) => ({ ...d, text: d.text.slice(0, 4000) })) });
}
...
async function upload(req, res, user) {
  ...
  await mutateDb((db) => {
    db.documents.push(doc); db.document_chunks.push(...chunks); db.document_facts.push(...analysis.facts); db.document_entities.push(...analysis.entities);
...
async function transform(req, res, user) {
  ...
  const docs = db.documents.filter((d) => ids.includes(d.id));
  if (!docs.length) return json(res, 404, { error: "Document not found" });
  const outputTypes = Array.isArray(body.outputTypes) ? body.outputTypes.slice(0, 14) : [body.outputType || "Citizen Simplifier"];
  const outputs = [];
  for (const outputType of outputTypes) {
    ...
    await mutateDb((wdb) => {
      ...
      wdb.generated_outputs.push(output); wdb.transformation_versions.push({ id: id("ver"), outputId: output.id, version: 1, content: output.content, createdBy: user.id, createdAt: now() });
...
async function compare(req, res) {
  const body = await bodyJson(req); const db = await readDb(); const [a, b] = body.documentIds || [];
  if (!a || !b) return json(res, 400, { error: "Two documentIds are required" });
  const factsA = db.document_facts.filter((f) => f.documentId === a).map((f) => f.claim); const factsB = db.document_facts.filter((f) => f.documentId === b).map((f) => f.claim);
  const onlyA = factsA.filter((x) => !factsB.some((y) => overlap(x, y) > 0.55)); const onlyB = factsB.filter((x) => !factsA.some((y) => overlap(x, y) > 0.55));
  return json(res, 200, { addedClauses: onlyB, removedClauses: onlyA, changedNumbers: diffRegex(factsA.join(" "), factsB.join(" "), /(?:INR|₹)?\s?\d[\d,]*(?:\s?(?:crore|lakh|%|days))?/gi), changedDates: diffRegex(factsA.join(" "), factsB.join(" "), /\d{1,2}\s+[A-Z][a-z]+\s+20\d{2}|20\d{2}/g) });
}

async function outputs(res) { const db = await readDb(); return json(res, 200, { outputs: db.generated_outputs.slice().reverse(), verification: db.verification_results }); }

async function reverify(req, res, outputId) {
  const db = await readDb(); const output = db.generated_outputs.find((o) => o.id === outputId);
  if (!output) return json(res, 404, { error: "Output not found" });
  const facts = db.document_facts.filter((f) => output.documentIds.includes(f.documentId)); const verification = verifyOutput(output.content, facts);
...
async function versionOutput(req, res, user, outputId) {
  const body = await bodyJson(req);
  const version = await mutateDb((db) => { const out = db.generated_outputs.find((o) => o.id === outputId); if (!out) return null; out.version += 1; out.content = String(body.content || out.content); out.updatedAt = now(); const ver = { id: id("ver"), outputId, version: out.version, content: out.content, createdBy: user.id, createdAt: now() }; db.transformation_versions.push(ver); return ver; });
  if (!version) return json(res, 404, { error: "Output not found" }); return json(res, 201, { version });
}

async function exportOutput(req, res, outputId) {
  const body = await bodyJson(req).catch(() => ({})); const db = await readDb(); const out = db.generated_outputs.find((o) => o.id === outputId);
  ...
  const file = path.join(exportDir, filename); await writeFile(file, content, "utf8");
  return json(res, 200, { filename, format, content });
}

async function analytics(res) {
  const db = await readDb(); const outputs = db.generated_outputs; const avg = (key) => Math.round(outputs.reduce((s, o) => s + (o[key] || 0), 0) / Math.max(1, outputs.length));
  return json(res, 200, { documentsProcessed: db.documents.length, transformationsGenerated: db.transformations.length || outputs.length, outputsGenerated: outputs.length, languagesUsed: [...new Set(outputs.map((o) => o.language))].length, averageFactualityScore: avg("factualityScore"), averageCitationCoverage: avg("citationCoverage"), byType: group(outputs, "outputType"), byLanguage: group(outputs, "language"), byDepartment: group(db.documents, "department"), monthlyUsage: group(outputs.map((o) => ({ month: o.createdAt.slice(0, 7) })), "month") });
}
...
async function requireUser(req, res) {
  ...
  const db = await readDb(); const user = db.users.find((u) => u.id === payload.sub);
...
async function bodyJson(req, maxMb = 2) {
  let data = "";
  for await (const chunk of req) { data += chunk; if (Buffer.byteLength(data) > maxMb * 1024 * 1024) throw new Error("Payload too large"); }
  try { return data ? JSON.parse(data) : {}; } catch { throw new Error("Invalid JSON payload"); }
}

async function serveStatic(res, pathname) {
  const clean = pathname === "/" ? "/index.html" : pathname; const target = path.normalize(path.join(webDir, clean));
  if (!target.startsWith(webDir)) return json(res, 403, { error: "Forbidden" });
  try { const bytes = await readFile(target); const type = target.endsWith(".css") ? "text/css" : target.endsWith(".js") ? "text/javascript" : "text/html"; res.writeHead(200, { "Content-Type": `${type}; charset=utf-8`, "Cache-Control": target.endsWith("index.html") ? "no-cache" : "public, max-age=3600" }); return res.end(bytes); }
  catch { const html = await readFile(path.join(webDir, "index.html")); res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }); return res.end(html); }
}
...
function rateLimited(req) {
  const key = req.socket.remoteAddress || "local"; const bucket = rate.get(key) || { count: 0, at: Date.now() }; if (Date.now() - bucket.at > 60_000) Object.assign(bucket, { count: 0, at: Date.now() }); bucket.count += 1; rate.set(key, bucket); return bucket.count > 240;
}
