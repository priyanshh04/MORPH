...
async function documents(res, user) {
  ...
  return json(res, 200, { documents: visible.map((d) => ({ ...d, text: d.text.slice(0, 4000) })) });
}
...
async function compare(req, res, user) {
  const body = await bodyJson(req); const db = await readDb(); const [a, b] = Array.isArray(body.documentIds) ? body.documentIds : [];
...
async function outputs(res, user) {
  ...
  return json(res, 200, { outputs: visible.slice().reverse(), verification: db.verification_results.filter((v) => outputIds.has(v.outputId)) });
}
...
async function versionOutput(req, res, user, outputId) {
  const body = await bodyJson(req); const content = String(body.content || "").trim();
...
async function serveStatic(res, pathname) {
  ...
  try {
    ...
    res.writeHead(200, { "Content-Type": `${type}; charset=utf-8`, "Cache-Control": target.endsWith("index.html") ? "no-cache" : "public, max-age=3600" });
...
function rateLimited(req) {
  ...
  if (rate.size > 10000) {
    ...
    for (const [address, entry] of rate) if (entry.at < cutoff) rate.delete(address);
  }
